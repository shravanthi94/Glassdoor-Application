...
router.get('/', adminCheckAuth, async(req, res) => {
    req.body.path = 'all_companies_get';
    console.log('all_companies_get-> Authentication Completed');
    kafka.make_request('adminCompany', req.body, (err, results) => {
      if (err) {
...
router.get('/:company_id/reviews', adminCheckAuth, async(req, res) => {
    req.body.path = 'company_reviews_get';
    req.body.company_id = req.params.company_id;
    console.log('company_reviews_get-> Authentication Completed');
    kafka.make_request('adminCompany', req.body, (err, results) => {
      if (err) {
        res.status(500).end('System Error');
      } else {
...
router.get('/:company_id/hired-applicants', adminCheckAuth, async(req, res) => {
    req.body.path = 'hired_applicants';
    req.body.company_id = req.params.company_id;
    console.log('hired_applicants-> Authentication Completed');
    kafka.make_request('adminCompany', req.body, (err, results) => {
      if (err) {
        res.status(500).end('System Error');
      } else {
...
router.get('/:company_id/applicant-demographics', adminCheckAuth, async(req, res) => {
    req.body.path = 'applicant_demographics';
    req.body.company_id = req.params.company_id;
    console.log('applicant_demographics-> Authentication Completed');
    kafka.make_request('adminCompany', req.body, (err, results) => {
      if (err) {
        res.status(500).end('System Error');
      } else {
