...
router.post('/primary', checkAuth, async (req, res) => {
  try {
    ...
    let primaryResume = await Student.findOneAndUpdate(
      { _id: req.body.studentId, 'resumes.isPrimary': true },
      { $set: { 'resumes.$.isPrimary': false } },
      { new: true },
    );

    let student = await Student.findOneAndUpdate(
      { _id: req.body.studentId, 'resumes._id': req.body.resumeId },
      { $set: data },
      { new: true },
    );
...
router.post('/remove', checkAuth, async (req, res) => {
  try {
    ...
    await Student.update(
      { _id: req.body.studentId },
      { $pull: { resumes: { _id: req.body.resumeId } } },
    );
