...
router.get('/aplicantdetail/:id', companyCheckAuth, async (req, res) => {
  try {
    const job = await Jobposting.findOne({
      $and: [{ email: req.company.email }, { 'applicants._id': req.params.id }],
    });

    const applicantIndex = job.applicants
      .map((item) => item.id)
  ...
  async (req, res) => {
    ...
    try {
      ...
      const job = await Jobposting.findOneAndUpdate(
        { email: req.company.email, 'applicants._id': req.params.applicantId },
        { $set: data },
        { new: true },
      );
      // console.log(job);
